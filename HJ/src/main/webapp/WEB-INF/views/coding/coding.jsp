...
<body>
	...
	<main>
		...
		<br>
		<section id="board">
			<h1><div>코딩 풀이 관련 게시물 리스트</div></h1><br>
			<c:if test="${listcount > 0}">
				<c:set var="num" value="${listcount-(page-1)*limit}" />
				<c:forEach var="F" items="${codingList}">
					<a class="item" href="${pageContext.request.contextPath}/.com/coding/${F.boardNum}">
						<div class="info">
				...
				<div class="center-block">
			 		<ul class="pagination justify-content-center">
			 			...
			 			<c:if test="${page > 1}">
			 				<li class="prev">
			 					<a class="pageNumber" href="coding?page=${page-1}"><i class="fa fa-chevron-left" aria-hidden="true"></i>;</a>
			 				</li>
			 			</c:if>
			 			...
			 			<c:forEach var="a" begin="${startpage}" end="${endpage}">
			 				...
			 				<c:if test="${a != page}">
			 					<li class="number">
			 						<a class="pageNumber" href="coding?page=${a}">${a}</a>
			 					</li>
			 				</c:if>
			 			</c:forEach>
			 			...
			 			<c:if test="${page < maxpage}">
			 				<li class="next">
			 					<a class="pageNumber" href="coding?page=${page+1}"><i class="fa fa-chevron-right" aria-hidden="true"></i></a>
			 				</li>
			 			</c:if>
			 		</ul>
			 	</div>
		...
		<div class="searchArea">
			<div></div>
			<div>
				<form action="coding" method="post" id="coding_Form">
					<div class="search">
			...
			<div class="write">
				<button type="button" id="codingWbtn" class="codingWbtn">글쓰기</button>
			</div>
		</div>
	</main>
</body>
