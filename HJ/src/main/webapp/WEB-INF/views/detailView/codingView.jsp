			...
			success: function (rdata) {
				if (rdata.listcount > 0) {
					...
					$(rdata.list).each(function () {
						...
						if (this.modiDate !== null && this.modiDate !== undefined) {
							rawDate = new Date(this.modiDate);
						}else {
							rawDate = new Date(this.creDate);
						}
...
<body>
	...
	<main>
		<input type="hidden" name="num" value="${codingData.boardNum}" id="boardNum">
		<article id="article">
				<div class="sub">
					<div class="back">
						<a href="${pageContext.request.contextPath}/page/coding"><i class="fa-solid fa-arrow-left-long"></i> 뒤로</a>
					</div>
				</div>
				<div class="item">
					<div class="info">
						<div class="title">
							<div class="category">
								<span>${codingData.cateName }</span>
							</div>
							<span>
								<h2>${codingData.boardTitle }</h2>
							</span>
						</div>
						<div class="etc">
							<div class="left">
								<div class="name">${codingData.id }</div>
								<div class="dot"><i class="fa-regular fa-circle" style="font-size: 5px;"></i></div>
								<div class="name">
									<c:choose>
									    <c:when test="${not empty codingData.creDate}">
									        <c:set var="formattedDate">
									        	<fmt:formatDate value="${codingData.creDate}" pattern="yyyy.MM.dd" />
									        </c:set>
								...
								<div class="dot"><i class="fa-regular fa-circle" style="font-size: 5px;"></i></div>
								<div class="viewCount"><i class="fa-regular fa-eye"></i>&nbsp;${codingData.boardView }</div>
							</div>
						</div>
					</div>
					<div class="customHtml"></div>
					<div class="content">${codingData.boardContent }</div>
					<div class="html"></div>
					<div class="customHtml"></div>
						...
						<div class="left">
							<div class="button">
								<c:if test="${countDown != 0}">
									<a href="${pageContext.request.contextPath}/page/coding/${countDown}">이전 글</a>
								</c:if>
							</div>
						</div>
						<div class="center">
							<div class="button">
								<a href="${pageContext.request.contextPath}/page/coding">목록</a>
							</div>
						</div>
						<div class="right">
							<div class="button">
								<c:if test="${countUp != 0}">
									<a href="${pageContext.request.contextPath}/page/coding/${countUp}">다음 글</a>
								</c:if>
							</div>
						</div>
					</div>
					...
					<div class="listAndEdit">
						...
						<div class="right">
							...
							<div class="button">
								<a href="${pageContext.request.contextPath}/page/updateBoard/${codingData.boardNum}">수정</a>
							</div>
