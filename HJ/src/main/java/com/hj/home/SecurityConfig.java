...
public class SecurityConfig  {
	...
	@Bean
	public AdminAccessDeniedHandler adminAccessDeniedHandler() {		//접속 제한
		return new AdminAccessDeniedHandler();
	}
	
	
	@Bean
	public AuthenticationSuccessHandler adminLoginSuccessHandler() {	//로그인 성공
		return new AdminLoginSuccessHandler();
	}
	
	
	@Bean
	public AuthenticationFailureHandler adminLoginFailHandler() {	//로그인 실패
		return new AdminLoginFailHandler();
	}
	
	
	@Bean
	public UserDetailsService adminDetailService() {				//계정 상세
		return new AdminUserDetailService();
	}
	
	@Bean
	public BCryptPasswordEncoder encodePassword() {			//비밀번호 인코딩
